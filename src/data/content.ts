...
export const daily = dailyJson as unknown as DailyData

/** Authored days only — drafts are never shown. */
export function daysForChapter(chapter: number): DailyDay[] {
  return daily.days.filter((d) => d.chapter === chapter && !d.draft)
}

export function dayByNumber(day: number): DailyDay | undefined {
